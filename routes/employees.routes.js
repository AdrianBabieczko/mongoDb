...
router.get('/employees/:id', (req, res) => {
  ...
  req.db.collection('employees').findOne({ _id: ObjectId(myId) }, (err, data) => {
    if (err) res.status(500).json({ message: err });
...
router.put('/employees/:id', (req, res) => {
  ...
  req.db.collection('employees').findOne({ _id: ObjectId(myId) }, (err, data) => {
    if (err) res.status(500).json({ message: err });
      ...
      req.db.collection('employees').updateOne({ _id: ObjectId(myId) }, { $set: { firstName: firstName, lastName: lastName } }, err => {
        if (err) res.status(500).json({ message: err });
        else res.json({ message: 'OK' });
      });
    }
...
router.delete('/employees/:id', (req, res) => {
  ...
  req.db.collection('employees').findOne({ _id: ObjectId(myId) }, (err, data) => {
    if (err) res.status(500).json({ message: err });
      ...
      req.db.collection('employees').deleteOne({ _id: ObjectId(myId) }, err => {
        if (err) res.status(500).json({ message: err });
        else res.json({ message: 'OK' });
      });
    }
